...
def export_model_to_hdf5_async(model, stimuli, filename, compression=9):
    ...
    if slurm_task_id is not None:
        logging.info(f"found SLURM task id: {slurm_task_id}")
        slurm_task_id = int(slurm_task_id)
        if slurm_task_id < 0 or slurm_task_id >= len(stimuli):
            logging.warning(f"SLURM task id {slurm_task_id} is out of range, ignoring")
        elif slurm_task_id not in indices:
            logging.warning(f"SLURM task id {slurm_task_id} is not in the list of indices, ignoring")
        else:
            logging.info(f"Using SLURM task id {slurm_task_id} to start with")
            indices = [slurm_task_id] + [i for i in indices if i != slurm_task_id]

        logging.info("SLURM task id is set, processing only one image")
        indices = indices[:1]
...
if task_id > 0:
    logging.info(f"SLURM task id {task_id} is set, exporting only")
    exit(0)
