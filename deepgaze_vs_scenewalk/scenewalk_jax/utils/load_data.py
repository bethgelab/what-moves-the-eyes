...
def find_data():
    ...
    if DATA_PATH is None:
        ...
        if os.path.exists(Path(__file__).parent / "../config.yml"):
            ...
            if os.path.exists(os.path.abspath("DATA")):
                ...
                raise Exception(
                    "You have not set up a folder from which to "
    ...
    print("Data loaded from " + from_where)
    return from_where
...
def autopopulate_data_path_dict(folder_path):
    ...
    for npy_path in load_paths:
        ...
        if not culprit is None:
            warnings.warn(
                "found data at " + npy_path + " but it is incomplete"
...
def populate_path_dict_from_yml(yml_path):
    ...
    for f in data_path_dict:
        ...
        if not culprit is None:
            warnings.warn(
                "found folder " + data_path_dict[f] + " but it is incomplete."
                "You are missing at least the *" + culprit + "* "
                "file."
            )
...
def setup_data_dict():
    ...
    """
    data_dict = {"x": None, "y": None, "dur": None, "im": None, "densities": None, "range": None, "meta": None}
    return data_dict
...
def load_data(data_ref):
    ...
    if all([x is None for x in loaded_dict.values()]):
        warnings.warn(
            "The string you passed is not a key in the data dict."
            "I thought maybe you passed a path: '" + folder + "' but I can't find the files there either."
        )
    return loaded_dict
...
def change_resolution(densities, new_size):
    ...
    result = []
    for d in densities:
        new_d = resize(d, dsize=(new_size, new_size), interpolation=INTER_CUBIC)
...
def _chop_list(lower, upper, li):
    ...
    for i in li:
        ...
        allim.append(np.array(im, dtype=object))
    return allim
